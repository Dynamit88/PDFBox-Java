package pdf.bouncycastle.jcajce.provider.keystore.bcfks;
...
import pdf.bouncycastle.asn1.ASN1InputStream;
import pdf.bouncycastle.asn1.ASN1ObjectIdentifier;
import pdf.bouncycastle.asn1.DERNull;
import pdf.bouncycastle.asn1.bc.EncryptedObjectStoreData;
import pdf.bouncycastle.asn1.bc.EncryptedPrivateKeyData;
import pdf.bouncycastle.asn1.bc.EncryptedSecretKeyData;
import pdf.bouncycastle.asn1.bc.ObjectData;
import pdf.bouncycastle.asn1.bc.ObjectDataSequence;
import pdf.bouncycastle.asn1.bc.ObjectStore;
import pdf.bouncycastle.asn1.bc.ObjectStoreData;
import pdf.bouncycastle.asn1.bc.ObjectStoreIntegrityCheck;
import pdf.bouncycastle.asn1.bc.PbkdMacIntegrityCheck;
import pdf.bouncycastle.asn1.bc.SecretKeyData;
import pdf.bouncycastle.asn1.bc.SignatureCheck;
import pdf.bouncycastle.asn1.cms.CCMParameters;
import pdf.bouncycastle.asn1.kisa.KISAObjectIdentifiers;
import pdf.bouncycastle.asn1.misc.MiscObjectIdentifiers;
import pdf.bouncycastle.asn1.misc.ScryptParams;
import pdf.bouncycastle.asn1.nist.NISTObjectIdentifiers;
import pdf.bouncycastle.asn1.nsri.NSRIObjectIdentifiers;
import pdf.bouncycastle.asn1.ntt.NTTObjectIdentifiers;
import pdf.bouncycastle.asn1.oiw.OIWObjectIdentifiers;
import pdf.bouncycastle.asn1.pkcs.EncryptedPrivateKeyInfo;
...
import pdf.bouncycastle.crypto.digests.SHA3Digest;
import pdf.bouncycastle.crypto.digests.SHA512Digest;
import pdf.bouncycastle.crypto.generators.PKCS5S2ParametersGenerator;
import pdf.bouncycastle.crypto.generators.SCrypt;
import pdf.bouncycastle.crypto.params.KeyParameter;
import pdf.bouncycastle.crypto.util.PBKDF2Config;
import pdf.bouncycastle.crypto.util.PBKDFConfig;
import pdf.bouncycastle.crypto.util.ScryptConfig;
import pdf.bouncycastle.jcajce.BCFKSLoadStoreParameter;
import pdf.bouncycastle.jcajce.BCFKSStoreParameter;
import pdf.bouncycastle.jcajce.BCLoadStoreParameter;
...
import pdf.bouncycastle.jce.interfaces.ECKey;
    ...
    extends KeyStoreSpi
{
    ...
    public Certificate[] engineGetCertificateChain(String alias)
    {
        ...
        if (ent != null)
        {
            ...
            {
                EncryptedPrivateKeyData encPrivData = EncryptedPrivateKeyData.getInstance(ent.getData());
                pdf.bouncycastle.asn1.x509.Certificate[] certificates = encPrivData.getCertificateChain();
    ...
    private Certificate decodeCertificate(Object cert)
    {
        if (helper != null)
        {
            try
            {
                ...
                return certFact.generateCertificate(new ByteArrayInputStream(pdf.bouncycastle.asn1.x509.Certificate.getInstance(cert).getEncoded()));
            }
            catch (Exception e)
            {
                return null;
    ...
    {
        pdf.bouncycastle.asn1.x509.Certificate[] certChain = new pdf.bouncycastle.asn1.x509.Certificate[chain.length];
        for (int i = 0; i != chain.length; i++)
        {
            certChain[i] = pdf.bouncycastle.asn1.x509.Certificate.getInstance(chain[i].getEncoded());
        }
        ...
        else if (parameter instanceof BCFKSLoadStoreParameter)
        {
            ...
            if (bcParam.getStoreSignatureKey() != null)
            {
                ...
                try
                {
                    ...
                    if (certs != null)
                    {
                        pdf.bouncycastle.asn1.x509.Certificate[] certificates = new pdf.bouncycastle.asn1.x509.Certificate[certs.length];
                        for (int i = 0; i != certificates.length; i++)
                        {
                            certificates[i] = pdf.bouncycastle.asn1.x509.Certificate.getInstance(certs[i].getEncoded());
                        }
        ...
        throws IOException, NoSuchAlgorithmException, CertificateException
    {
        ...
        else if (integrityCheck.getType() == ObjectStoreIntegrityCheck.SIG_CHECK)
        {
            ...
            try
            {
                pdf.bouncycastle.asn1.x509.Certificate[] certificates = sigCheck.getCertificates();
                if (validator != null)
                {

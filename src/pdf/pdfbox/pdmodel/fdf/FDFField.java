...
import pdf.pdfbox.pdmodel.interactive.action.PDAdditionalActions;
    ...
    public FDFField(Element fieldXML) throws IOException
    {
        ...
        for (int i = 0; i < nodeList.getLength(); i++)
        {
            ...
            if (node instanceof Element)
            {
                ...
                {
                    setValue(pdf.pdfbox.util.XMLUtil.getNodeValue(child));
                }
                else if (child.getTagName().equals("value-richtext"))
                {
                    setRichText(new COSString(pdf.pdfbox.util.XMLUtil.getNodeValue(child)));
                }
